...
- Streaming subscriptions cover level-one equities/options/futures/futures options/forex, chart equity/futures, NYSE/NASDAQ/options books, and screener equity/option; book streams use one shared `Book` payload model and `BookField` selector for `NYSE_BOOK`, `NASDAQ_BOOK`, and `OPTIONS_BOOK`
