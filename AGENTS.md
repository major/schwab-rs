...
- `schwab-agent account` and `schwab-agent account --positions` expose `balances.true_cash_status` plus `balances.true_cash` when Schwab provides a margin-safe cash source; downstream SGOV sweep or funding automation must use `true_cash` only when the status is `verified` and must not treat buying power, available funds, cash available for trading or withdrawal, option buying power, or similar capacity fields as margin-safe cash
