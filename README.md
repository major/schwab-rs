...
`schwab-agent account` and `schwab-agent account --positions` include a balance `true_cash_status` field for downstream funding decisions. Use `true_cash` only when `true_cash_status` is `verified`; treat `unavailable` as a prompt for confirmation before selling SGOV or assuming idle cash. Do not treat `buying_power`, `available_funds`, `cash_available_for_trading`, `cash_available_for_withdrawal`, option buying power, or similar capacity fields as margin-safe cash for SGOV sweep and entry funding decisions.
...
|---|---:|---|
| `cli` | Yes | Enables the bundled `schwab-agent` binary and CLI-only dependencies, including clap parsing, shell completions, browser opening, config directory lookup, preview digests, and `RUST_LOG` tracing setup. |
