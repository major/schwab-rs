...
- **Order builder** - typed equity helpers, single-leg option helpers, OCO, first-triggers-second order composition, and repeat-order conversion
...
schwab-agent analyze SPY
schwab-agent analyze AAPL MSFT --points 5
schwab-agent analyze SPY --expected-move --dte 45
```

Command-specific `--help` output includes copyable examples for the main market, option, and analyze workflows so agents can discover usage without leaving the terminal. `analyze` combines live quote data with TA values derived from completed candles; derived fields include `analysis.derived.price_basis`, `price_basis_value`, and `price_basis_timestamp` so downstream callers do not confuse live quote timestamps with the price used for daily indicator comparisons. `market history --from` and `--to` accept `YYYY-MM-DD`, RFC3339, or epoch milliseconds. Date-only values use inclusive UTC calendar-day boundaries. Option chain and screen help list the valid `--type` values: `call`, `put`, and `all`. Option screen numeric filters reject non-finite values such as `NaN` and infinity before making API calls, and screen output serializes numeric values through the crate's active `Number` type so default and `decimal` builds stay consistent.
...
|---|---:|---|
| `cli` | Yes | Enables the bundled `schwab-agent` binary and CLI-only dependencies, including clap parsing, browser opening, config directory lookup, preview digests, and `RUST_LOG` tracing setup. |
...
Offline tests include `cli` feature-gated compiled-binary smoke checks for `schwab-agent` help output, clap usage errors, structured JSON error output, and hermetic dry-run order payloads. Live Schwab API tests remain gated behind `test_online`.
