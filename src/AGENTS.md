...
- Subscription methods cover level-one equities, options, futures, futures options, forex, chart equity, chart futures, `subscribe_nyse_book`, `subscribe_nasdaq_book`, `subscribe_options_book`, screener equity, and screener option
...
- Message dispatch maps supported service names (level-one, chart, book (`NYSE_BOOK`, `NASDAQ_BOOK`, `OPTIONS_BOOK`), and screener) into the matching `StreamData` variants and skips unknown services with a warning
