...
The public library remains rooted in `src/lib.rs`. The `schwab-agent` CLI is a separate binary target under `src/bin/schwab-agent/`, gated by the default `cli` feature so library consumers can build with `default-features = false`; it can use CLI config files, environment variables, sanitized setup status and doctor output, machine-readable schema discovery, `RUST_LOG` stderr tracing, opt-in JSON usage errors through `SCHWAB_AGENT_JSON_ERRORS`, structured JSON output, process exit codes, explicit local order draft flags (`--dry-run` or `--preview`, choose one per command), owner-only saved preview files, compatibility token paths, shell completion generation with stderr diagnostics for write failures, command-specific help examples, symbol-filtered order discovery for targeted open-order checks, and CLI-specific validation such as rejecting invalid market history date arguments before auth/API calls, rejecting non-finite option screen filters, and enforcing normalized contract-type filters, but those behaviors must not leak into public library modules.
...
- Compiled-binary smoke tests live in `tests/cli_smoke.rs`, run only when the `cli` feature is enabled, and use `assert_cmd` plus `predicates` for offline help, discovery schema, doctor/config health output, shell completion, clap error, structured error JSON, and hermetic implicit plus explicit dry-run order checks
