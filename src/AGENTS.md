...
The public library remains rooted in `src/lib.rs`. The `schwab-agent` CLI is a separate binary target under `src/bin/schwab-agent/`, gated by the default `cli` feature so library consumers can build with `default-features = false`; it can use CLI config files, environment variables, sanitized setup status and doctor output, machine-readable schema discovery including high-frequency aliases (`quote`, `history`, `orders`, `positions`) and `stock buy`/`stock sell` migration stubs, margin-safe account output fields (`balances.true_cash_status` and verified `balances.true_cash` for SGOV sweep or funding decisions), `RUST_LOG` stderr tracing, opt-in JSON usage errors through `SCHWAB_AGENT_JSON_ERRORS`, structured JSON output, process exit codes, explicit local order draft flags (`--dry-run` or `--preview`, choose one per command), owner-only saved preview files, compatibility token paths, shell completion generation through `completions` plus the singular `completion` alias with stderr diagnostics for write failures, command-specific help examples, symbol-filtered order discovery for targeted open-order checks, canonical `--order-id` lifecycle arguments with compatibility validation, and CLI-specific validation such as rejecting invalid market history date arguments before auth/API calls, rejecting non-finite option screen filters, and enforcing normalized contract-type filters, but those behaviors must not leak into public library modules.

`schwab-agent analyze` output can include live quote timestamps beside daily TA values computed from completed candle closes. Keep `analysis.derived.price_basis`, `price_basis_value`, and `price_basis_timestamp` populated when changing TA output so agents can tell which price produced derived distance and signal values.
...
- Renovate dependency policy is inherited from the org-level shared config; validate with `npx --yes --package renovate renovate-config-validator` and do not add a repo-local `renovate.json` unless this crate needs a specific override
- Compiled-binary smoke tests live in `tests/cli_smoke.rs`, run only when the `cli` feature is enabled, and use `assert_cmd` plus `predicates` for offline help, discovery schema, doctor/config health output, bash/zsh/fish/PowerShell shell completion, clap error, structured error JSON, migration-hint JSON, command aliases, and hermetic implicit plus explicit dry-run order checks
