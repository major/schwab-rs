...
- Subscription methods cover level-one equities, options, futures, futures options, forex, chart equity, and chart futures
...
- Message dispatch maps supported service names (level-one and chart) into the matching `StreamData` variants and skips unknown services with a warning
