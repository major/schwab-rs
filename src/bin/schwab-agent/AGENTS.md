...
- **repeat**: Repeat an existing order by positive order ID. Requires `--account`; use `--order-id` as the canonical spelling. Positional order IDs remain accepted for compatibility, and if both forms are supplied they must match. Fetches the source order, converts it with `schwab::OrderBuilder::try_from_order`, and runs the rebuilt payload through the normal order workflow. Supports `SINGLE`, `TRIGGER`, and `OCO` orders with equity or option legs when Schwab can convert them. Use `--save-preview` for the recommended digest workflow. Unsupported historical shapes return `order.validation_failed`; use `order place-raw` when the complex payload needs manual adaptation.
- **cancel**: Cancel by order ID, requires `--account`; use `--order-id` as the canonical spelling. Positional order IDs remain accepted for compatibility, and if both forms are supplied they must match. Includes post-cancel verification via GET and only reports `verified` once the fetched status is `CANCELED`.
...
Account summary balances include `cash_balance`, `true_cash`, and `true_cash_status` for SGOV sweep and funding decisions when Schwab provides them. `true_cash_status="verified"` means `true_cash` came from a margin-safe Schwab cash field: margin accounts use `currentBalances.cashBalance` (Cash & Sweep Vehicle) first, then `initialBalances.totalCash` and `initialBalances.cashBalance` as fallbacks, and cash accounts use `currentBalances.cashBalance` with `currentBalances.totalCash` fallback, then `initialBalances.cashBalance` when current balances do not provide either field. `true_cash_status="unavailable"` means Schwab did not provide a usable true-cash source in the rendered balance data. `true_cash_status="not_applicable"` is reserved for account shapes where true cash does not apply. Do not treat `buying_power`, `available_funds`, `cash_available_for_trading`, `cash_available_for_withdrawal`, option buying power, or similar capacity fields as margin-safe cash.
...
Commands output raw JSON data payloads directly (no wrapper). Application errors output an `ErrorBody` JSON object with `code`, `message`, `category`, `retryable`, and `hint` fields. Set `SCHWAB_AGENT_JSON_ERRORS=1` to render clap usage errors as the same JSON shape on stdout with stable `usage.*` codes and actionable hints for unknown commands, invalid values, missing required arguments, and argument conflicts; default human-readable clap stderr remains unchanged when the variable is unset or false-like. `auth.refresh_token_invalid` uses exit code 3 and category `auth` when Schwab returns OAuth `invalid_grant` for token refresh, and its hint points to full re-authentication because another refresh cannot fix a revoked or expired refresh token. `completions` and its singular alias `completion` are the only raw stdout exceptions because shell completion scripts must not be JSON-wrapped; completion generation write failures emit a short stderr diagnostic and exit non-zero.
