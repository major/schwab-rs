...
Cash account `currentBalances` include `cash_balance` (`cashBalance`) when Schwab provides Cash & Sweep Vehicle-style cash in that snapshot. Margin account `currentBalances` do not model true cash because their available-funds and buying-power fields are capacity values; margin-safe cash for CLI output comes from modeled margin `initialBalances` cash fields when present.
